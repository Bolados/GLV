unit bcParser;
{
  Copyright Suavi Ali Demir, 1999-2004
...
type
  EbcParserError = class(Exception); // create a new exception class
...
type
  ...
  protected
    ...
    function CreateParseTree(var formula: string; var Node: TNode) : boolean;
...
implementation
uses math    {$IFDEF EVALVERSION}, windows {$ENDIF};

type //a record that ties a fucntions memory address to the number of parameters it takes.
...
begin
   ...
   *)
   ...
   begin
      FNode.Free; //free doesn't assign nil to the pointer...
      FNode := nil;
      raise EbcParserError.Create('Syntax error in expression ' + FExpression);
   end;
...
begin
     ...
     begin
          ...
          if n<0 then exit; //at any moment if expression is valid we cannot have more ) then (
     end;
     Result:= (n = 0); //true if number of brackets matches
end;
//------------------------------------------------------------------------------
procedure RemoveOuterBrackets(var formula: string); //removes unncessary outer brackets in an expression
var temp: string;
    Len: integer;
begin
     ...
     begin
        ...
        if CheckBrackets(temp) then //if we did not screw up then assign to the return value
             formula:= temp;
        Len:= Length(temp);
     end;
end;
...
//------------------------------------------------------------------------------
function TbcParser.CreateParseTree(var formula: string; var Node: TNode) : boolean;
var num : NUMBER;
...
begin
     ...
     RemoveOuterBrackets(formula); //remove unnecessary brackets
     //we should first remove brackets, and then check the formula length that might be modified
     //while removing brackets.

     if (not Length(formula)>0) or (not CheckBrackets(formula)) then
        exit; //if length is zero or brackets does not match then return false.
     ...
     begin //attach a number node in the structure
          ...
          begin
            ...
            begin
              if (not CreateParseTree(paramLeft, leftNode)) or
                 (not CreateParseTree(paramRight, rightNode)) then
                 exit;
...
var i, funcIndex, BracketLevel, paramStart, Len : integer;
    c, nextCh : char;
...
var i, funcIndex, paramStart, Len: integer;
        //temp_ : array [1..MAX_NAME_LEN] of char;
        temp : String; 
begin
  Len:= Length(formula);
...
var i, funcIndex, BracketLevel, paramStart, Len, pIndex, nParams : integer;
...
begin
  ...
  if not IsValidName(AnsiUpperCase(name)) then begin
    raise EbcParserError.Create(name + ' is not a valid function name.');
  end;

  if (FOneParamFuncs.Find(name, index)) or (FTwoParamFuncs.Find(name, index)) then
    raise EbcParserError.Create('Function ' + name + ' already exists.')
  else
...
begin
  if (FVariables.Find(name, index)) then //to use TStringList.Find the list must be sorted.
    ...
    raise EbcParserError.Create('Variable ' + name + ' is not found.');
end;
//------------------------------------------------------------------------------
...
procedure Register;
begin
  RegisterComponents('BestCode', [TbcParser]);
end;
